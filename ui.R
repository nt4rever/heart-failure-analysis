              ...
              column(
                ...
                box(
                  ...
                  tags$div(class = "plot__container", tags$p(tags$b("Note:")), tags$ul(
                    tags$li("ExerciseAngina: exercise-induced angina [Y: Yes, N: No] Angina is a type of chest pain caused by reduced blood flow to the heart")
                  ))
                ),
                box(
                  title = "ST_Slope",
                  ...
                  tags$div(class = "plot__container", tags$p(tags$b("Note:")), tags$ul(
                    tags$li("ST_Slope: the slope of the peak exercise ST segment [Up: upsloping, Flat: flat, Down: downsloping] Mainly Flat in (Males) and relatively Up in (Females)")
                  ))
                ),
                box(
                  ...
                  tags$div(class = "plot__container", tags$p(tags$b("Observations:")), tags$ul(
                    tags$li("We may need to do some t-test to check whether the true means difference is equal to 0 or Not. we can scipy package to do that, but for now, let's move on...")
                  ))
                ),
              ),
              column(
                ...
                box(
                  ...
                  tags$div(class = "plot__container", tags$p(tags$b("Observations:")), tags$ul(
                    tags$li("It is somehow the same pattern for Male and Female relative to their counts!"),
                    tags$li("We have many of 0 values in the Cholesterol var!!"),
                    tags$li("Let's use lmplot to see the regression line of each group."),
                    tags$li("I think we have to remove the 0 values in the Cholesterol var and check again!")
                  ))
                ),
              )
            ))
